...
*/
__global__ void invert_mapping(cudaTextureObject_t input, /* original */
    ...
    if (point_id < npoints) {
        for (i = 0; i < nfeatures; i++)
            output[point_id + npoints * i] =
                wrap::ptx::tex2Ds32<float>(input, i, point_id);
    }
    return;
}
/* ----------------- invert_mapping() end --------------------- */
    ...
    ***/
    ...
    if (threadIdx.x < nfeatures * nclusters) {
        ...
        for (int i = 0; i < (THREADS_PER_BLOCK); i++) {
            float val =
                wrap::ptx::tex2Ds32<float>(t_features_flipped, dim_id, (point_id - threadIdx.x) + i);
