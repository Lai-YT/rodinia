...
float *GICOV_CUDA(int grad_m, int grad_n, float *host_grad_x,
    ...
    resDescX.res.linear.desc.x = 32; // float is 32 bits
    ...
    cudaCreateTextureObject(&tex_grad_x, &resDescX, &texDesc, NULL);
    ...
    resDescY.res.linear.desc.x = 32;
    resDescY.res.linear.sizeInBytes = grad_mem_size;

    cudaTextureObject_t tex_grad_y = 0;
    cudaCreateTextureObject(&tex_grad_y, &resDescY, &texDesc, NULL);
...
__global__ void dilate_kernel(int img_m, int img_n, int strel_m, int strel_n,
                              cudaSurfaceObject_t dilated, cudaTextureObject_t t_img) {
    ...
    // Store the maximum value found (use 2D indexing)
    surf2Dwrite<float>(max, dilated, j * sizeof(float), i);
}
...
float *dilate_CUDA(int max_gicov_m, int max_gicov_n, int strel_m, int strel_n) {
    ...
    wrap::cuda::SurfaceObject<float> device_img_dilated;
    wrap::cuda::malloc2DSurfaceObject(&device_img_dilated, max_gicov_n, max_gicov_m);
    ...
    PROFILE((
        dilate_kernel<<<num_blocks, threads_per_block>>>(
            max_gicov_m, max_gicov_n, strel_m, strel_n, device_img_dilated.surf, tex_img)
    ));
    ...
    wrap::cuda::memcpy2DFromSurfaceObject(host_img_dilated, &device_img_dilated, max_gicov_n, max_gicov_m);

    // Cleanup memory
    cudaDestroyTextureObject(tex_img);
    cudaFree(device_gicov);
    wrap::cuda::freeSurfaceObject(&device_img_dilated);
