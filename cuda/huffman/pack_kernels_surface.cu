...
__global__ void pack2(cudaSurfaceObject_t srcData, unsigned int *cindex,
    ...
    dw = surf2Dread<unsigned int>(srcData, 0, tid); // load the first dword from
						    // srcData[] row
    ...
    for (i = 1; i < bitsize / 32;
        ...
        // load next dword from srcData[] row
        dw = surf2Dread<unsigned int>(srcData, i * sizeof(unsigned int), tid);
