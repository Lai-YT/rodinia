...
#include <sys/time.h>
#include "vlc_kernel_sm64huff_surface.cu"
#include "cpuencode.h"
...
void runVLCTest(char *file_name, uint num_block_threads, uint num_blocks) {
    ...
    unsigned int *d_sourceData, *d_destDataPacked;
    unsigned int *d_codewords, *d_codewordlens;
    unsigned int *d_cw32, *d_cw32len, *d_cw32idx, *d_cindex, *d_cindex2;
    wrap::cuda::SurfaceObject<unsigned int> d_destData;

    CUDA_SAFE_CALL(cudaMalloc((void **)&d_sourceData, mem_size));
    CUDA_SAFE_CALL(wrap::cuda::malloc2DSurfaceObject(&d_destData,
                                                     num_block_threads, num_blocks));
    ...
    CUDA_SAFE_CALL(cudaMemcpy(d_codewordlens, codewordlens,
                              NUM_SYMBOLS * symbol_type_size,
                              cudaMemcpyHostToDevice));
    CUDA_SAFE_CALL(wrap::cuda::memcpy2DToSurfaceObject(&d_destData, destData,
                                                       num_block_threads, num_blocks));
...
#endif
            d_destData.surf, d_cindex); // testedOK2
    }
...
//////////////////* END KERNEL *///////////////////////////////////
    ...
    pack2<<<num_scan_elements / 16, 16>>>(
        d_destData.surf, d_cindex, d_cindex2,
    ...
    CUDA_SAFE_CALL(wrap::cuda::freeSurfaceObject(&d_destData));
