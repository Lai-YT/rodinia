 ...
 **-------------------------------------------------------
 */
__global__ void Fan1(cudaSurfaceObject_t m_cuda, cudaSurfaceObject_t a_cuda, int Size, int t) {
    ...
    if (x >= Size - 1 - t)
        return;
    float a_tt = surf2Dread<float>(a_cuda, t * sizeof(float), t);
    float a_xt = surf2Dread<float>(a_cuda, t * sizeof(float), x + t + 1);
    float m_xt = a_xt / a_tt;
    surf2Dwrite<float>(m_xt, m_cuda, t * sizeof(float), x + t + 1);
}
 ...
 **-------------------------------------------------------
 */
__global__ void Fan2(cudaSurfaceObject_t m_cuda, cudaSurfaceObject_t a_cuda, float *b_cuda, int Size,
                     int j1, int t) {
    int x = blockDim.x * blockIdx.x + threadIdx.x;
    ...
    if (y >= Size - t)
        return;
    float a_xy = surf2Dread<float>(a_cuda, (y + t) * sizeof(float), x + 1 + t);
    float m_xt = surf2Dread<float>(m_cuda, t * sizeof(float), x + 1 + t);
    float a_ty = surf2Dread<float>(a_cuda, (y + t) * sizeof(float), t);
    a_xy -= m_xt * a_ty;
    surf2Dwrite<float>(a_xy, a_cuda, (y + t) * sizeof(float), x + 1 + t);
    if (y == 0) {
        b_cuda[x + 1 + t] -= m_xt * b_cuda[t];
    }
}
...
void ForwardSub() {
    ...
    wrap::cuda::malloc2DSurfaceObject(&a_cuda, Size, Size);

    cudaMalloc((void **)&b_cuda, Size * sizeof(float));

    // copy memory to GPU
    wrap::cuda::memcpy2DToSurfaceObject(&m_cuda, m, Size, Size);
    wrap::cuda::memcpy2DToSurfaceObject(&a_cuda, a, Size, Size);
    ...
    gettimeofday(&time_start, NULL);
    for (t = 0; t < (Size - 1); t++) {
        Fan1<<<dimGrid, dimBlock>>>(m_cuda.surf, a_cuda.surf, Size, t);
        cudaThreadSynchronize();
        Fan2<<<dimGridXY, dimBlockXY>>>(m_cuda.surf, a_cuda.surf, b_cuda, Size, Size - t, t);
        cudaThreadSynchronize();
        checkCUDAError("Fan2");
    }
    // end timing kernels
    ...
    wrap::cuda::memcpy2DFromSurfaceObject(m, &m_cuda, Size, Size);
    wrap::cuda::memcpy2DFromSurfaceObject(a, &a_cuda, Size, Size);
    cudaMemcpy(b, b_cuda, Size * sizeof(float), cudaMemcpyDeviceToHost);
    wrap::cuda::freeSurfaceObject(&m_cuda);
    wrap::cuda::freeSurfaceObject(&a_cuda);
    cudaFree(b_cuda);
}
