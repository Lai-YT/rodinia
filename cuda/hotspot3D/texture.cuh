...
/// \note Make sure the \p width and \p height are not exchanged, the correct
/// order is `float data[height][width]`.
cudaError_t malloc2DSurfaceObject(SurfaceObject *surfObj, size_t width,
                                  size_t height) {
  cudaError_t err;

  std::memset(surfObj, 0, sizeof(SurfaceObject));
...
cudaError_t malloc1DSurfaceObject(SurfaceObject *surfObj, size_t width) {
  cudaError_t err;
...
/// \note Make sure the \p width and \p height are not exchanged, the correct
/// order is `float data[height][width]`.
cudaError_t memcpy2DToSurfaceObject(SurfaceObject *surfObj, void *data,
                                    size_t width, size_t height) {
  // The width in memory in bytes of the 2D array pointed to by devPtr,
  // including padding. We don't have any padding.
  return cudaMemcpy2DToArray(surfObj->devPtr, 0, 0, data, width * sizeof(float),
                             width * sizeof(float), height, cudaMemcpyDefault);
}

cudaError_t memcpy1DToSurfaceObject(SurfaceObject *surfObj, void *data,
                                    size_t width) {
  return cudaMemcpy2DToArray(surfObj->devPtr, 0, 0, data, width * sizeof(float),
                             width * sizeof(float), 1, cudaMemcpyDefault);
}
...
/// \note Make sure the \p width and \p height are not exchanged, the correct
/// order is `float data[height][width]`.
cudaError_t memcpy2DFromSurfaceObject(void *dst, SurfaceObject *surfObj,
                                      size_t width, size_t height) {
  return cudaMemcpy2DFromArray(dst, width * sizeof(float), surfObj->devPtr, 0,
                               0, width * sizeof(float), height,
                               cudaMemcpyDefault);
}

cudaError_t memcpy1DFromSurfaceObject(void *dst, SurfaceObject *surfObj,
                                      size_t width) {
  return cudaMemcpy2DFromArray(dst, width * sizeof(float), surfObj->devPtr, 0,
                               0, width * sizeof(float), 1,
                               cudaMemcpyDefault);
}
...
cudaError_t freeSurfaceObject(SurfaceObject *surfObj) {
  cudaError_t err;
